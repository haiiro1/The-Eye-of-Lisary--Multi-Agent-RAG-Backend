...
def builder_node(state: AgentState, config: RunnableConfig):
    session_id = config["configurable"].get("thread_id", "default_session")
    query = get_human_query(state["messages"])
    # Recuperación de fragmentos limitada por relevancia técnica
    context = get_rag_tool().search(f"Ficha: {query}") if debe_buscar_en_rag(query) else ""
    expert = CharBuilder(session_id=session_id, chat_history=state["messages"], callbacks=config.get("callbacks", []))
    result = expert.run(user_input=query, language=state.get("language", "es"), extra_info=context, config=config)
    return {"messages": [AIMessage(content=f"[CHAR_BUILDER]: {result['answer']}")], "selected_agents": _pop_agent(state)}
...
def aggregator_node(state: AgentState, config: RunnableConfig):
    try:
        ...
        # Limpieza de bloques de pensamiento para optimizar el contexto del agregador
        clean_history = []
        ...
        prompt = ChatPromptTemplate.from_template("""
            ...
            REGLAS CRÍTICAS:
            - Responde estrictamente en el IDIOMA: {idioma}.
            - NO uses etiquetas <think> ni comentarios técnicos de los agentes.
            - Mantén el tono místico y usa negritas para términos de reglas.

            VISIONES RECOPILADAS:
            {clean_history}
        ...
        # Eliminación de razonamientos internos en la respuesta de salida
        ...
        return {"messages": [AIMessage(content="El Ojo está nublado por energías extrañas. Inténtalo de nuevo.")], "selected_agents": []}